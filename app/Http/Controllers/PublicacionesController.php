...
class PublicacionesController extends Controller
{
    public function show($name)
    {
        if (strcmp('todas', $name) != 0) {
            ...
            $anios = Publicaciones::select(DB::raw('count(anio_publicacion) as total'), 'active', 'tipo', 'anio_publicacion')
                ->whereBelongsTo($categoria)->where('active', true)
                ->groupBy('anio_publicacion')->get()
                ->pluck('total', 'anio_publicacion');
            ...
            $anios = Publicaciones::select(DB::raw('count(anio_publicacion) as total'), 'active', 'tipo', 'anio_publicacion')
                ->where('active', true)->where('tipo', 'publicación')
                ->groupBy('anio_publicacion')->get()
                ->pluck('total', 'anio_publicacion');
    ...
    public function colecciones($name)
    {
        ...
        $anios = Publicaciones::select(DB::raw('count(anio_publicacion) as total'), 'active', 'tipo', 'anio_publicacion')
            ->where('active', true)->where('tipo', 'colección')
            ->groupBy('anio_publicacion')->get()
            ->pluck('total', 'anio_publicacion');
